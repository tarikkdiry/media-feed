...
@Injectable()
export class SharePointCrudService {

  // Data
  jsonHeader = 'application/json; odata=verbose';
  xmlheaders = new Headers({'Content-Type': 'text/xml'});
  ...
  readFromApi(SPApiUrl: string): Promise<any> {
     const localOptions: RequestOptions = this.options;
     localOptions.headers.append('X-Requested-With', 'XMLHttpRequest');
      // tslint:disable-next-line:prefer-const
      let url = this.baseUrl + SPApiUrl;
      return this.http.get(url, localOptions).toPromise().then(function (resp: Response) {
      return resp.json();
    });
  }
